...
// This user_level configuration tests the KernelTap element, which accesses
// Linux's ethertap device (or, equivalently, *BSD's /dev/tun* devices). These
// devices let user_level programs trade packets with the kernel.  You will
// need to run it as root.
//
// This configuration should work on FreeBSD, OpenBSD, and Linux. It should
// produce a stream of 'tap_ok' printouts if all goes well. 
// On OpenBSD, you may need to run
//   route add 1.0.0.0 -interface 1.0.0.1
// after starting the Click configuration.
//
// See test_tun.click for the KernelTun version of this configuration, which
...
c[0] -> Strip(14)
	-> ch :: CheckIPHeader
	-> ipc :: IPClassifier(icmp echo_reply, icmp echo, -)
	-> IPPrint(tap_ok)
	-> d :: Discard;
  ipc[1] -> Print(tap_ping)
	-> ICMPPingResponder
	-> out;
  ipc[2] -> d;
  ch[1] -> Print(tap_bad) -> d;
